...
// Default seconds a provider may go without a heartbeat before the reaper
// thread discards it. For the second demo, run with "3" as a command-line
// argument so the 5-second heartbeat is too slow to keep the list server
// registered (no rebuild needed).
constexpr int DEFAULT_PROVIDER_TIMEOUT_SECONDS{10};

// How often the reaper wakes up to scan the registry. This MUST be shorter than
// the list server's 5-second heartbeat interval: if the reaper polled at the same
// 5-second period it would keep sampling each provider at the same age (phase lock)
// and could miss the window where a provider is briefly stale, so a short timeout
// like 3s would never actually fire. Polling once per second makes the timeout
// value alone decide the outcome, independent of when each server started.
constexpr int REAPER_INTERVAL_SECONDS{1};

// Runs forever on its own thread: every REAPER_INTERVAL_SECONDS it locks the
// registry and removes any provider whose last heartbeat is older than timeoutSeconds.
void reapUnresponsiveProviders(SharedRegistry& registry, int timeoutSeconds) {
    while (true) {
        ...
        for (auto serviceIt{registry.services.begin()}; serviceIt != registry.services.end();) {
            ...
            for (auto providerIt{providers.begin()}; providerIt != providers.end();) {
                ...
                if (silence > std::chrono::seconds(timeoutSeconds)) {
                    ...
                    providerIt = providers.erase(providerIt);
                } else {
                    ++providerIt;
                }
            }

            // Drop the service entry entirely once it has no providers left.
            if (providers.empty()) {
                serviceIt = registry.services.erase(serviceIt);
            } else {
                ++serviceIt;
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    // Optional first argument overrides the heartbeat timeout (in seconds).
    // For the second demo, run: ./name_server_binary 3
    int timeoutSeconds{DEFAULT_PROVIDER_TIMEOUT_SECONDS};
    if (argc >= 2) {
        const int parsed{std::atoi(argv[1])};
        if (parsed > 0) {
            timeoutSeconds = parsed;
        } else {
            std::cerr << "Invalid timeout '" << argv[1] << "', using default "
                      << DEFAULT_PROVIDER_TIMEOUT_SECONDS << "s\n";
        }
    }
    ...
    std::cout << "Provider heartbeat timeout: " << timeoutSeconds << "s\n";

    // Start the reaper thread, which prunes providers that stop sending heartbeats.
    std::thread reaperThread{reapUnresponsiveProviders, std::ref(registry), timeoutSeconds};
    reaperThread.detach();
