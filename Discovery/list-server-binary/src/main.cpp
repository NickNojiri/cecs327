...
bool registerWithNameServer(const std::string& nameServerHost,
    ...
    // We track and increment the message ID by hand. The name server suppresses
    // duplicate message IDs on a connection, so every heartbeat must use a new one.
    ...
    std::cout << "Registered with name server; starting heartbeat loop\n";

    // Registration succeeded. Keep the same connection open and, every 5 seconds,
    // send a Heartbeat so the name server knows this provider is still alive.
    while (true) {
        std::this_thread::sleep_for(std::chrono::seconds(5));

        ++messageId;
        const std::vector<std::uint8_t> heartbeatPayload{
            buildHeartbeatRequest(serviceName, providerId)
        };
        if (!sendAll(fd, frameSuppressibleMessage(clientId.value(), messageId, heartbeatPayload))) {
            std::cerr << "Failed to send heartbeat to name server\n";
            closeSocket(fd);
            return false;
        }
        ...
        if (!heartbeatResponse.has_value()) {
            std::cerr << "Lost connection to name server during heartbeat\n";
            closeSocket(fd);
            return false;
        }
        ...
        const auto heartbeatOpcode{
            heartbeatOpcodeValue.has_value()
                ? static_cast<NameServerResponseOpcode>(heartbeatOpcodeValue.value())
                : NameServerResponseOpcode::Error
        };

        if (heartbeatOpcode == NameServerResponseOpcode::Ok) {
            std::cout << "Heartbeat #" << messageId << " acknowledged by name server\n";
        } else {
            // The name server may have already discarded us (e.g. the timeout was
            // shorter than our heartbeat interval). Report it but keep trying.
            std::optional<std::string> errorMessage{heartbeatReader.readString()};
            std::cerr << "Heartbeat #" << messageId << " not OK: "
                      << (errorMessage.has_value() ? errorMessage.value() : "unknown")
                      << '\n';
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Registering with name server at " << argv[1] << ':' << nameServerPort << '\n';
